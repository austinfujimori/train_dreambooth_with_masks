...
def get_background_pixels_and_bbox(pil_img, blur_radius=0, bbox_padding=10, mask_parts=None):
    ...
    options = mp.tasks.vision.ImageSegmenterOptions(
        base_options=mp.tasks.BaseOptions(model_asset_path="selfie_multiclass_256x256.tflite"),
...
def main(args):
    ...
    os.makedirs("pixel_masks", exist_ok=True)
    os.makedirs("latent_masks", exist_ok=True)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                face_loss_start_step = 10
                if step < face_loss_start_step:
                    # Compute model input without masking
                    if vae is not None:
                    ...
                    for pixel_value in pixel_values:
                        test = get_background_pixels_and_bbox(pixel_value, 0, 10, ["hair", "clothing"])
                        print
                        masks.append( get_background_pixels_and_bbox(pixel_value, 0, 10, ["hair", "clothing"])[0] )
                        
                        
                    # masks = create_masks(pixel_values)
                    ...
                    # Resize the masks to match the spatial dimensions of the model predictions
                    masks = F.interpolate(masks.unsqueeze(1).float(), size=model_pred.shape[-2:], mode='bilinear', align_corners=False).squeeze(1)
                
                    # Apply mask weighting to the model predictions and target values
                    model_pred = model_pred * masks
                    target = target * masks
                    

                    # Save the pixel masks
                    for i, mask in enumerate(masks):
                        mask_np = mask.detach().cpu().numpy()
                        mask_np = (mask_np * 255).astype(np.uint8)
                        mask_image = Image.fromarray(mask_np)
                        mask_image.save(f"pixel_masks/mask_step_{step}_batch_{i}.png")
                
                    # Save the decoded latent masks
                    latent_masks = vae.encode(masks.unsqueeze(1)).latent_dist.sample()
                    decoded_masks = vae.decode(latent_masks).sample
                    for i, mask in enumerate(decoded_masks):
                        mask_np = mask.detach().cpu().permute(1, 2, 0).numpy()
                        mask_np = (mask_np * 255).astype(np.uint8)
                        mask_image = Image.fromarray(mask_np)
                        mask_image.save(f"latent_masks/mask_step_{step}_batch_{i}.png")
                ...
                if args.snr_gamma is None:
                    if step < face_loss_start_step:
                        ...
                        #loss = torch.mean((model_pred.float() - target.float()) ** 2 * latent_masks)
                        #new one
                        loss = F.mse_loss(model_pred.float(), target.float(), reduction="none")
                        loss = (loss * masks).mean() / masks.mean()
                    
                # Compute instance loss
                # if args.snr_gamma is None:
                #     # squared_diff = (model_pred.float() - target.float()) ** 2
                #     # print("Squared differences:")
                #     # print(squared_diff)
                    
                #     # masked_diff = squared_diff * (latent_masks)
                #     # print("Masked differences:")
                #     # print(masked_diff)
                    
                #     # total_diff = masked_diff.mean()
                #     # print("Loss:")
                #     # print(total_diff)
                ...
                #     #AUSTIN
                #     loss = (model_pred.float() - target.float()) ** 2 * (latent_masks)
