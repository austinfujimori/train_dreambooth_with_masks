...
def get_background_pixels_and_bbox(pil_img, blur_radius=0, bbox_padding=10, mask_parts=None):
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.image_transforms_no_norm = transforms.Compose(
            [
                transforms.Resize(size, interpolation=transforms.InterpolationMode.BILINEAR),
                transforms.CenterCrop(size) if center_crop else transforms.RandomCrop(size),
                transforms.ToTensor(),
        ...
        self.image_transforms_norm = transforms.Compose(
            [
                transforms.Normalize([0.5], [0.5]),
            ]
        )
    ...
    def __getitem__(self, index):
        ...
        clothesmask_array = np.array(clothesmask)
        facemask_array = np.array(facemask)
        facemask = Image.fromarray(clothesmask_array + facemask_array)
        ...
        # Set clothing regions to gray (eroding it sets it to white)
        # probably could be optimized
        # binary erosion causes the mask to be completely white and completely black (binary)
        # so can't put in the gray clothes white face mask, instead we make clothes gray after
        # So we pass in the combined mask of face + clothes to get our eroded mask, then
        # create a new mask of pixels in both eroded mask and original face mask, set to white
        # and also create a new mask of pixels in both eroded mask and original clothes mask, set to gray
        ...
        # new method, 
        # when we add noise to the instance images, we treat it as if the clothes are a part of the BG
        # like we did in in one of the tests. So this means that our blurred mask is just the blurred mask of the face,
        # as 1-blurred mask is where we will be replacing with noise. 
        # But unlike that test, instead of also just taking the loss of the face area, we will still take loss
        # of the face area + clothes area with the clothes mask weighted a little less.

        # so we just blur the face
        combined_mask = eroded_face # + eroded_clothes
        combined_mask_image = Image.fromarray(combined_mask)
        combined_mask_tensor = self.image_transforms_mask(combined_mask_image)

        # then we store another one for the mask loss
        combined_mask_loss = eroded_face + eroded_clothes
        combined_mask_loss_image = Image.fromarray(combined_mask_loss)
        combined_mask_loss_tensor = self.image_transforms_mask(combined_mask_loss_image)
        
                
        # BLUR THE EDGES OF THE COMBINED MASK
        blurred_combined_mask = np.copy(combined_mask)
        gray_image = cv2.cvtColor(blurred_combined_mask, cv2.COLOR_RGB2GRAY)
        edges = cv2.Canny(gray_image, threshold1=100, threshold2=200)
        edge_mask = edges > 0
        blurred_edges = cv2.GaussianBlur(blurred_combined_mask, (5, 5), sigmaX=0)
        blurred_combined_mask[edge_mask] = blurred_edges[edge_mask]
        ...
        blurred_clothes_mask = np.copy(eroded_clothes)
        ...
        example["masks"] = combined_mask_loss_tensor
        ...
        if self.class_data_root:
            ...
            # Add class prompt IDs and attention mask
...
def main(args):
    ...
    # so we can remove the clothes_mask stuff as we will be treating
    # the clothes area as the BG area
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if vae is not None:
                    ...
                    # let's try encoding the masks and still use our noise adding methods
                    masks = vae.encode(masks).latent_dist.sample()
                    masks = masks * vae.config.scaling_factor
                ...
                if args.snr_gamma is None:
                    ...
                    # we are using latent masks this test so no need for this stuff
                    ...
                    # old and won't use
                    masked_loss = F.mse_loss(model_pred * new_encoded_masks, target * new_encoded_masks, reduction="none")
                    masked_loss = masked_loss.mean(dim=list(range(1, len(masked_loss.shape))))
                    loss = (masked_loss * mse_loss_weights).mean()
