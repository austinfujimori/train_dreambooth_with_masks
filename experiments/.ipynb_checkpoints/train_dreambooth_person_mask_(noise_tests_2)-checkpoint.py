...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # noise_scale = 1.0
        # background_noise = noise_scale * background_noise
        ...
        # ADD SMALL NOISE TO WHOLE METHOD
        # instance_image_tensor = instance_image_tensor * facemask_tensor.float() + background_noise * background_mask.float() + small_noise
        ...
        # struct_elem = np.ones((15, 15), dtype=np.bool_)
        ...
        # instance_image_tensor = instance_image_tensor * facemask_tensor.float() + background_noise * background_mask.float() + small_noise * edgemask_tensor.float()


        # DILATE BG METHOD
        # struct_elem = np.ones((20, 20), dtype=np.bool_)
        # mask_array = np.any(np.array(backgroundmask) != [0, 0, 0], axis=-1)
        # dilated_mask = binary_dilation(mask_array, struct_elem)
        # dilated_edge_mask = np.zeros_like(np.array(instance_image))
        # dilated_edge_mask[dilated_mask] = [255, 255, 255]
        # dilated_edge_mask_image = Image.fromarray(dilated_edge_mask)
        # dilated_tensor = self.image_transforms_mask(dilated_edge_mask_image)
        
        # instance_image_tensor = instance_image_tensor * facemask_tensor.float() + background_noise * dilated_tensor.float()
        ...
        # BACKGROUND NOISE DILATION METHOD V2
        noise = torch.randn_like(instance_image_tensor)
        noise_min = noise.min()
        noise_max = noise.max()
        # Normalize the noise between -0.5 and 0.5
        background_noise = (noise - (noise_max + noise_min) / 2) / ((noise_max - noise_min) / 2)
        # tried 0.2
        noise_scale = 0.2
        background_noise = noise_scale * background_noise
