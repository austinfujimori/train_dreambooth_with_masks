...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # blur the background a little bit so theres like no outline around the person
        # tried 0, 5
        backgroundmask, _bbox = get_background_pixels_and_bbox(instance_image, mask_parts=['background', 'others'], blur_radius=0, bbox_padding=0)

        facemask_tensor = self.image_transforms_mask(facemask)
        background_mask = self.image_transforms_mask(backgroundmask)
        ...
        # ADDING NOISE TO BACKGROUND METHOD
        noise = torch.randn_like(instance_image_tensor)
        # norm the noise between 0 and 1 (which is the range of the image values bc of the toTensor)
        noise_min = noise.min()
        noise_max = noise.max()
        normalized_noise = (noise - noise_min) / (noise_max - noise_min)
        # scale of noise we want to add
        # tried 1, 0.5, 0.2, 0.05
        noise_scale = 0.05
        normalized_noise = noise_scale * normalized_noise

        # Adding noise on top of instance image after multiplied with blurred/no blur BG mask
        # instance_image_tensor = instance_image_tensor + normalized_noise * background_mask.float()

        # tried blurring the facemask a little (2) and a lot (10)
        facemask_blurred, bbox = get_background_pixels_and_bbox(instance_image, mask_parts=['hair', 'skin', 'face', 'clothing'], blur_radius=0, bbox_padding = 0)
        facemask_blurred_tensor = self.image_transforms_mask(facemask_blurred)
        instance_image_tensor = instance_image_tensor * facemask_blurred_tensor.float()  + normalized_noise # * background_mask.float()
        

        
        # # REMOVING BACKGROUND METHOD
        # instance_image_tensor = instance_image_tensor * (background_mask.float())
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # print(f"new encoded_masks shape: {new_encoded_masks.shape}")
                ...
                if args.snr_gamma is None:
                    ...
                    #     .reshape(
                    #         model_pred.shape[0], 1, model_pred.shape[2], model_pred.shape[3]
                    #     )
                    #     # 8 = 512/64
                    # )
