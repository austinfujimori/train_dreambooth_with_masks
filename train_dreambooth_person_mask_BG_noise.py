...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Add noise to BG
        background_mask = 1 - facemask
        noise = torch.randn_like(instance_image)
        background_mask = background_mask.unsqueeze(0)
        instance_image = instance_image + noise * background_mask.float()

        # save_dir = "noised"
        # os.makedirs(save_dir, exist_ok=True)
        
        # # Save the image (assuming you're looping over multiple instances)
        # filename = f"transformed_image_{idx}.png"
        # vutils.save_image(example["instance_images"], os.path.join(save_dir, filename))
